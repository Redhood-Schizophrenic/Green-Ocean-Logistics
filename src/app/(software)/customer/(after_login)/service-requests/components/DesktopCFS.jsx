...
export default function DesktopCFS() {
	...
	const StatusColor = ({ status }) => {
		switch (status) {
			case 'approved':
				return 'bg-[var(--primary)] text-[var(--background)]'
			case 'gol-approved':
				return 'bg-blue-500 text-[var(--background)]'
			case 'rejected':
				return 'bg-red-700 text-[var(--background)]'
			default:
				return 'bg-yellow-600 text-[var(--background)]'
		}
	}
		...
		<section className="w-full min-h-screen">
			...
			<div id="requests-container" className="grid gap-4">
				{filteredRequests.map((request, index) => (
					<div key={index} id="requests-card" className="border rounded-xl p-4">
